...
ball_images = [
    ...
    pygame.image.load(os.path.join(image_path, "balloon21.png")),
    pygame.image.load(os.path.join(image_path, "balloon31.png")),
    pygame.image.load(os.path.join(image_path, "balloon41.png"))]

# 공 크기에 따른 최초 스피드
ball_speed_y = [-7, -6, -5, -4]  # index 0, 1, 2, 3
...
balls.append({
    ...
    "to_x": 2,  # x축 이동방향, -3 이면 왼쪽으로, 3이면 오른쪽으로
    "to_y": -3,  # y축 이동방향
    "init_spd_y": ball_speed_y[0]})  # 공의 y축 최초속도
...
while running:
    ...
    for ball_idx, ball_val in enumerate(balls):
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                if ball_img_idx < 3:
                    ...
                    balls.append({
                        ...
                        "to_x": -2,  # x축 이동방향, -3 이면 왼쪽으로, 3이면 오른쪽으로
                        "to_y": -3,  # y축 이동방향
                        "init_spd_y": ball_speed_y[ball_img_idx+1]})  # 공의 y축 최초속도
        ...
        break  # 안쪽 for 문에서 break를 만나면 이 break 실행, 2중 for문을 한번에 탈출
    ...
    # 남은 총알
    rest_bullet = 3 - len(weapons)
    str_rest_bullet = game_font.render(
        "Rest Bullet : {0}" .format(int(rest_bullet)), True, (255, 255, 255))
    screen.blit(str_rest_bullet, (440, 10))

    # 경과 시간 계산
    elapsed_time = (pygame.time.get_ticks() - start_ticks) / 1000
    timer = game_font.render("Time : {0}" .format(
